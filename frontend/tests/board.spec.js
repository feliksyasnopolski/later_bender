...
test.describe('task board rendered acceptance', () => {
  ...
  test('creates a task and exercises narrow layout', async ({ page }) => {
    ...
    await expect(dialog.getByLabel('Context')).toBeVisible()
    await expect(dialog.getByLabel('Intended direction')).toBeVisible()
    ...
    await createForm.getByLabel('Project').selectOption({ index: 1 })
    await dialog.getByLabel('Status').selectOption('ready')
    await dialog.getByLabel('Priority').selectOption('high')
    await dialog.getByLabel('Tags').fill('browser, modal')
    await dialog.getByLabel('Context').fill('Created through the modal')
    await dialog.getByLabel('Intended direction').fill('Keep the workflow focused')
