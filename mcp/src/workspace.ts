...
const resourceRequirements = {
  min_cpus: z.number().int().positive().optional(),
  min_memory_bytes: bytes.optional(),
  min_disk_bytes: bytes.optional()
};
const resourceLimits = z.object({ cpus: z.number().int().positive(), memory_bytes: bytes, disk_bytes: bytes });
const capabilityFlags = z.object({ internet: z.boolean(), gpu: z.boolean(), nested_virtualization: z.boolean() });
...
const transcriptEvent = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("file_imported"), sequence: z.number().int().positive(), occurred_at: timestamp, workspace: opaqueRef, file: opaqueRef, path: z.string() }),
  z.object({
    ...
    state: executionState, exit_code: z.number().int().nullable(), terminating_signal: z.string().nullable(), stdout_preview: streamProjection, stderr_preview: streamProjection
  }),
...
export function registerWorkspaceTools(server: McpServer): void {
  server.registerTool("get_workspace_capabilities", { description: `Discover the environments, architectures, operating system, shell, resource limits, and useful execution capabilities available for a Workspace. Identifiers are open strings and no infrastructure implementation details are exposed. ${workspaceFailure}`, inputSchema: {}, outputSchema: {
    default_environment: z.string(), default_architecture: z.string(), environments: z.array(z.string()), architectures_by_environment: z.record(z.string(), z.array(z.string())), os, shell: z.string(),
    resources: z.object({ default: resourceLimits, max: resourceLimits }), capabilities: capabilityFlags
  }, annotations: readOnly }, noRuntime);

  server.registerTool("create_workspace", { description: `Create a transient Workspace using optional minimum resource requirements. An empty object requests the normal useful default; requirements are minimums, not exact operator configuration, and are never silently substituted. ${workspaceFailure}`, inputSchema: { label: z.string().optional(), environment: z.string().optional(), architecture: z.string().optional(), resources: z.object(resourceRequirements).optional() }, outputSchema: { workspace }, annotations: create }, noRuntime);
  ...
  server.registerTool("put_file_in_workspace", { description: `Copy exact canonical Later Bender File bytes into a Workspace at a Workspace-relative path. If path is omitted, the canonical filename is used at /workspace root. Paths cannot escape /workspace and the import is recorded in the transcript. ${workspaceFailure}`, inputSchema: { workspace: opaqueRef, file: opaqueRef, path: z.string().optional(), overwrite: z.boolean().default(false) }, outputSchema: { workspace: opaqueRef, file: opaqueRef, path: z.string(), byte_size: bytes, sha256: z.string() }, annotations: create }, noRuntime);
  server.registerTool("read_workspace_file", { description: `Read a bounded text-ish generated file using Workspace-relative paths and optional line locators. Without a locator, return an initial chunk and continuation metadata. This is not a generic binary transport; non-text content returns not_text. ${workspaceFailure}`, inputSchema: { workspace: opaqueRef, path: z.string(), locator: lineLocator.optional() }, outputSchema: { workspace: opaqueRef, path: z.string(), content: z.string(), media_type: z.string(), range: z.object({ kind: z.literal("lines"), start: z.number().int().positive(), end: z.number().int().positive(), byte_start: bytes, byte_end: bytes, complete: z.boolean() }), next_cursor: cursor }, annotations: readOnly }, noRuntime);
  ...
  server.registerTool("read_workspace_transcript", { description: `Read the ordered transient Workspace activity ledger, including creation, File imports, executions, and promotions. Sequence numbers are monotonic per Workspace; the transcript also provides execution discovery after chat migration. ${workspaceFailure}`, inputSchema: { workspace: opaqueRef, from_sequence: z.number().int().positive().optional(), to_sequence: z.number().int().positive().optional(), limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }, outputSchema: { events: z.array(transcriptEvent), next_cursor: cursor }, annotations: readOnly }, noRuntime);
