...
const projectSummary = z.object({ slug: z.string(), shorthand: z.string(), name: z.string() });
...
const locator = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("pages"), start: z.number().int().positive(), end: z.number().int().positive() })
]);
const citation = z.object({ file: z.string(), representation: z.string(), locator });
const task = taskSummary.extend({ context: z.string().nullable(), intended_direction: z.string().nullable(), related_notes: z.array(relatedNote), related_files: z.array(relatedFile), citations: z.array(citation) });
...
const note = noteBase.extend({ body: z.string(), related_task_refs: z.array(z.string()), related_tasks: z.array(relatedTask), related_files: z.array(relatedFile), citations: z.array(citation) });
...
const projectCreateOutput = { project: z.object({ slug: z.string(), shorthand: z.string(), updated_at: timestamp }) };
const projectUpdateOutput = { project: z.object({ slug: z.string(), updated_at: timestamp }) };
...
const manageFilesOutput = {
  results: z.array(z.object({ operation: z.literal("delete"), ref: z.string(), status: z.enum(["succeeded", "not_found"]) }))
};
...
const fileSearchResult = z.object({ kind: z.literal("file"), ref: z.string(), number: z.number(), filename: z.string(), media_type: z.string(), project: fileProject, tags: z.array(z.string()), snippet: z.string(), match: z.object({ representation: z.string(), locator }), created_at: timestamp, updated_at: timestamp }).strict();
const searchResult = z.discriminatedUnion("kind", [taskSearchResult, noteSearchResult, fileSearchResult]);
const cursorOutput = z.string().nullable();
const source = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("file"), ref: z.string() }),
  z.object({ kind: z.literal("archive_entry"), ref: z.string(), path: z.string() })
]);
const textRead = z.object({ kind: z.literal("text"), source, representation: z.string(), coordinate: z.literal("lines"), locator: z.object({ kind: z.literal("lines"), start: z.number().int().positive(), end: z.number().int().positive() }), media_type: z.string().nullable(), content: z.string(), metadata: z.record(z.string(), z.unknown()) });
const pdfRead = z.object({ kind: z.literal("pdf"), source, representation: z.string(), coordinate: z.literal("pages"), locator: z.object({ kind: z.literal("pages"), start: z.number().int().positive(), end: z.number().int().positive() }), media_type: z.string().nullable(), content: z.string(), metadata: z.record(z.string(), z.unknown()) });
const metadataRead = z.object({ kind: z.literal("metadata"), source, representation: z.string(), coordinate: z.null(), locator: z.null(), media_type: z.string().nullable(), metadata: z.record(z.string(), z.unknown()) });
const readResult = z.discriminatedUnion("kind", [textRead, pdfRead, metadataRead]);
const readBatchResult = z.union([z.object({ ref: z.string(), read: readResult }), z.object({ ref: z.string(), error: z.enum(["not_found", "invalid_locator"]) })]);
...
const projectView = (value: any): any => value && typeof value === "object" ? { slug: value.slug, shorthand: value.shorthand, name: value.name } : value;
const taskView = (value: any): any => { if (!value || typeof value !== "object") return value; const { id: _id, ...task } = value; const view: any = { ...task, project: projectView(task.project) }; if (Array.isArray(task.related_notes)) view.related_notes = task.related_notes.map((related: any) => ({ ...related, project: related.project === null ? null : projectView(related.project) })); if (Array.isArray(task.related_tasks)) view.related_tasks = task.related_tasks.map(taskView); return view; };
const noteView = (value: any): any => { if (!value || typeof value !== "object") return value; const { related_task_ids: _ids, ...note } = value; const relatedTasks = note.related_tasks?.map(taskView) || []; return { ...note, project: note.project === null ? null : projectView(note.project), related_task_refs: relatedTasks.map((task: any) => task.ref), related_tasks: relatedTasks }; };
const noteSummaryView = (value: any): any => value && typeof value === "object" ? { ...value, project: value.project === null ? null : projectView(value.project) } : value;
...
const fileSearchView = (value: any): any => ({ kind: "file", ref: value.ref, number: value.number, filename: value.filename, media_type: value.media_type, project: projectSearchView(value.project), tags: value.tags, snippet: value.snippet, match: value.match, created_at: value.created_at, updated_at: value.updated_at });
...
const noteScopeInput = z.object({ scope: z.enum(["global", "project", "all"]).optional(), project: z.string().optional(), tags: z.array(z.string()).optional(), sort: z.enum(["created_at", "updated_at"]).optional(), order: z.enum(["asc", "desc"]).optional(), limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }).superRefine((value, context) => {
...
export function registerTools(server: McpServer, api: LaterBenderApi): void {
  server.registerTool("list_projects", { description: "Browse Projects visible to the authenticated user in deterministic creation order. Continue with the opaque next_cursor until it is null.", inputSchema: { limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }, outputSchema: { projects: z.array(project), next_cursor: cursorOutput }, annotations: readAnnotations }, (input) => safeObject(() => api.listProjects(input).then((page) => ({ projects: page.items, next_cursor: page.next_cursor }))));
  ...
  server.registerTool("create_project", { description: "Create a project. Omit slug to let Later Bender generate one from the name; the returned slug is the Project identity for subsequent calls.", inputSchema: { name: z.string(), slug: z.string().optional(), shorthand: z.string().optional(), description: z.string().optional() }, outputSchema: projectCreateOutput, annotations: createAnnotations }, (input) => safe("project", () => api.createProject(input).then((value: any) => ({ slug: value.slug, shorthand: value.shorthand, updated_at: value.updated_at }))));
  server.registerTool("update_project", { description: "Update mutable Project fields by slug.", inputSchema: { slug: z.string(), name: z.string().optional(), description: z.string().optional() }, outputSchema: projectUpdateOutput, annotations: updateAnnotations }, ({ slug, ...payload }) => safe("project", () => api.updateProject(slug, payload).then((value: any) => ({ slug: value.slug, updated_at: value.updated_at }))));

  server.registerTool("list_tasks", { description: "Browse Tasks in a known project using structured filters and canonical board-position order. Use search_memory for approximate textual recall across Tasks, Notes, and Files. Continue with the opaque next_cursor until it is null; tags matches all supplied tags.", inputSchema: { project: z.string(), status: statuses.optional(), priority: priorities.optional(), tags: z.array(z.string()).optional(), limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }, outputSchema: { tasks: z.array(taskSummary), next_cursor: cursorOutput }, annotations: readAnnotations }, ({ project: projectSlug, tags, limit, cursor, ...filters }) => safeObject(() => api.listTasks(projectSlug, { ...filters, tags: tags?.join(","), limit: limit?.toString(), cursor }).then((page) => ({ tasks: (page.items as any[]).map(taskView), next_cursor: page.next_cursor }))));
  ...
  server.registerTool("create_task", { description: "Create an actionable Task in a project. Omitted status and priority use Later Bender defaults (backlog and normal). position controls canonical board order; omitted position appends. Unknown tags are created automatically. related_note_ids links Notes that materially contributed to or contextualize the Task. citations are exact evidence pointers into canonical Files, distinct from broader related_files relationships.", inputSchema: { project: z.string(), title: z.string(), status: statuses.optional(), position: z.number().int().optional(), priority: priorities.optional(), context: z.string().optional(), intended_direction: z.string().optional(), tags: z.array(z.string()).optional(), related_note_ids: z.array(z.number().int().positive()).optional(), citations: z.array(citationInput).optional() }, outputSchema: taskMutationOutput, annotations: createAnnotations }, ({ project: projectSlug, ...payload }) => compactMutation("task", () => api.createTask(projectSlug, payload)));
  server.registerTool("update_task", { description: "Partially update a Task by external ref. Omitted tags, related_note_ids, or citations preserve existing values; [] clears them; a non-empty array replaces them exactly. citations are exact evidence pointers into canonical Files, distinct from broader related_files relationships. Unknown tags are created automatically.", inputSchema: { ref: z.string(), ...taskFields }, outputSchema: taskMutationOutput, annotations: updateAnnotations }, ({ ref, ...payload }) => compactMutation("task", () => api.updateTask(ref, payload)));

  server.registerTool("list_notes", { description: "Browse compact Note summaries in a known scope with deterministic server-side ordering. Use search_memory instead for approximate recall. Defaults to global scope and updated_at desc; continue with the opaque next_cursor until it is null.", inputSchema: noteScopeInput, outputSchema: { notes: z.array(noteSummary), next_cursor: cursorOutput }, annotations: readAnnotations }, (input) => safeObject(() => api.listNotes(input).then((page) => ({ notes: (page.items as any[]).map(noteSummaryView), next_cursor: page.next_cursor }))));
  ...
  server.registerTool("create_note", { description: "Store durable non-actionable context such as an idea, decision, finding, constraint, hypothesis, possible direction, or discussion result. Omit project for global context; provide a project slug for project-specific context. citations are exact evidence pointers into canonical Files, distinct from broader related_files relationships. Unknown tags are created automatically.", inputSchema: { title: z.string(), body: z.string(), project: z.string().optional(), tags: z.array(z.string()).optional(), citations: z.array(citationInput).optional() }, outputSchema: noteMutationOutput, annotations: createAnnotations }, (input) => compactMutation("note", () => api.createNote(input)));
  server.registerTool("update_note", { description: "Update Note metadata or deliberately replace whole fields by global ID. Omitted project, tags, or citations preserve them; empty arrays clear them. Use edit_note for targeted body maintenance.", inputSchema: { id: z.number().int().positive(), title: z.string().optional(), body: z.string().optional(), project: z.string().nullable().optional(), tags: z.array(z.string()).optional(), citations: z.array(citationInput).optional() }, outputSchema: noteMutationOutput, annotations: updateAnnotations }, ({ id, ...payload }) => compactMutation("note", () => api.updateNote(id, payload)));
  const noteEditOperation = z.discriminatedUnion("operation", [z.object({ operation: z.literal("append"), text: z.string() }), z.object({ operation: z.literal("replace"), old_text: z.string().min(1), new_text: z.string() })]);
  server.registerTool("edit_note", { description: "Apply a small atomic batch of targeted body edits to a Note by global ID. Operations run in order; replace requires old_text to occur exactly once. expected_updated_at prevents overwriting a newer Note.", inputSchema: { id: z.number().int().positive(), operations: z.array(noteEditOperation).min(1).max(100), expected_updated_at: timestamp.optional() }, outputSchema: noteMutationOutput, annotations: updateAnnotations }, ({ id, ...payload }) => compactMutation("note", () => api.editNote(id, payload)));
  server.registerTool("delete_note", { description: "Permanently delete a Note by global numeric ID. This is genuinely destructive and intended for erroneous or throwaway state; it is not archival or supersession. Existing durable Notes that are merely outdated should normally be updated or replaced semantically.", inputSchema: { id: z.number().int().positive() }, outputSchema: noteDeletionOutput, annotations: { ...updateAnnotations, idempotentHint: false } }, ({ id }) => safe("note", () => api.deleteNote(id)));

  server.registerTool("list_files", { description: "Browse immutable canonical Files in a project with deterministic server-side ordering. Defaults to created_at desc; continue with the opaque next_cursor until it is null. Tags match all supplied tags.", inputSchema: { project: z.string(), tags: z.array(z.string()).optional(), sort: z.enum(["created_at", "updated_at", "filename", "size"]).optional(), order: z.enum(["asc", "desc"]).optional(), limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }, outputSchema: { files: z.array(fileSummary), next_cursor: cursorOutput }, annotations: readAnnotations }, ({ project: projectSlug, tags, limit, cursor, sort, order }) => safeObject(() => api.listFiles(projectSlug, { tags: tags?.join(","), limit: limit?.toString(), cursor, sort, order }).then((page) => ({ files: (page.items as any[]).map(fileView), next_cursor: page.next_cursor }))));
  ...
  server.registerTool("manage_files", { description: "Manage canonical File lifecycle operations. Supports permanently deleting a File by its external ref; File content remains immutable.", inputSchema: { operations: z.array(fileManagementOperation).min(1).max(100) }, outputSchema: manageFilesOutput, annotations: { ...updateAnnotations, idempotentHint: false } }, ({ operations }) => safe("results", () => Promise.all(operations.map(async (operation) => {
    try {
      ...
      return { operation: operation.operation, ref: operation.ref, status: "succeeded" as const };
    } catch (error) {
      if (error instanceof ApiError && error.code === "not_found") return { operation: operation.operation, ref: operation.ref, status: "not_found" as const };
      throw error;
    }
  }))));

  server.registerTool("read_file", { description: "Read a bounded model-consumable File representation. If representation is omitted, auto selects the most useful available representation. Results identify the canonical File, effective representation, coordinate system, and effective locator; images return typed metadata only.", inputSchema: { ref: z.string(), representation: z.string().optional(), locator: locator.optional() }, outputSchema: { read: readResult }, annotations: readAnnotations }, ({ ref, representation, locator }) => safe("read", () => api.readFile(ref, representation, locator)));
  server.registerTool("read_files", { description: "Read multiple Files in request order with typed results and per-item not_found or invalid_locator errors. Omitted representation means auto; returned locators support bounded follow-up reads and citations.", inputSchema: { reads: z.array(z.object({ ref: z.string(), representation: z.string().optional(), locator: locator.optional() })).min(1).max(100) }, outputSchema: { results: z.array(readBatchResult) }, annotations: readAnnotations }, ({ reads }) => safe("results", () => api.readFiles(reads)));
  server.registerTool("list_archive", { description: "Browse a canonical archive File in deterministic path order without promoting members into Files. path optionally scopes to a directory/prefix and depth bounds traversal; continue with the opaque next_cursor until it is null.", inputSchema: { ref: z.string(), path: z.string().optional(), depth: z.number().int().nonnegative().max(100).optional(), limit: z.number().int().positive().max(100).optional(), cursor: z.string().optional() }, outputSchema: { entries: z.array(archiveEntry), next_cursor: cursorOutput }, annotations: readAnnotations }, ({ ref, ...input }) => safeObject(() => api.listArchive(ref, input).then((page) => ({ entries: page.items, next_cursor: page.next_cursor }))));
  server.registerTool("read_archive_entry", { description: "Read one exact member of one canonical archive File without creating a File. Results identify the archive ref and member path with typed text, PDF, or metadata output. Nested archives remain entries until explicitly read or extracted.", inputSchema: { ref: z.string(), path: z.string(), representation: z.string().optional(), locator: locator.optional() }, outputSchema: { read: readResult }, annotations: readAnnotations }, ({ ref, path, representation, locator }) => safe("read", () => api.readArchiveEntry(ref, path, representation, locator)));
  server.registerTool("extract_archive_entry", { description: "Explicitly promote one exact member of a canonical archive File into a new immutable canonical File. This is the only archive member promotion operation; no automatic archive explosion occurs. The parent archive is unchanged, provenance records archive_ref and entry_path, and the default project is the parent File project.", inputSchema: { ref: z.string(), path: z.string(), project: z.string().optional(), filename: z.string().optional(), tags: z.array(z.string()).optional(), related_task_refs: z.array(z.string()).optional(), related_note_ids: z.array(z.number().int().positive()).optional() }, outputSchema: fileMutationOutput, annotations: createAnnotations }, ({ ref, path, ...rest }) => compactMutation("file", () => api.extractArchiveEntry(ref, { path, ...rest })));
  server.registerTool("search_memory", { description: "Search Later Bender durable state across Tasks, Notes, and readable File representations. File matches return canonical refs and locators suitable for read_file.", inputSchema: searchInput, outputSchema: { results: z.array(searchResult) }, annotations: readAnnotations }, (input) => safe("results", () => api.searchMemory(input).then((response) => response.results.map((entry: any) => entry.kind === "task" ? taskSearchView(entry) : entry.kind === "note" ? noteSearchView(entry) : fileSearchView(entry)))));
