...
const note = noteBase.extend({ body: z.string(), related_task_ids: z.array(z.number()), related_tasks: z.array(relatedTask), related_files: z.array(relatedFile), citations: z.array(citation) });
...
export function registerTools(server: McpServer, api: LaterBenderApi): void {
  ...
  server.registerTool("update_note", { description: "Partially update a Note by global ID. Omitted project, tags, or citations preserve them; empty arrays clear them. citations are exact evidence pointers into canonical Files, distinct from broader related_files relationships.", inputSchema: { id: z.number().int().positive(), title: z.string().optional(), body: z.string().optional(), project: z.string().nullable().optional(), tags: z.array(z.string()).optional(), citations: z.array(citationInput).optional() }, outputSchema: noteMutationOutput, annotations: updateAnnotations }, ({ id, ...payload }) => compactMutation("note", () => api.updateNote(id, payload)));
  ...
  server.registerTool("read_file", { description: "Read a bounded model-consumable File representation. If representation is omitted, auto selects the most useful available representation. Textual reads expose line coordinates; PDF reads preserve page coordinates. Locators are intended for bounded follow-up reads and citations. Images currently return metadata only.", inputSchema: { ref: z.string(), representation: z.string().optional(), locator: locator.optional() }, outputSchema: { read: z.record(z.string(), z.unknown()) }, annotations: readAnnotations }, ({ ref, representation, locator }) => safe("read", () => api.readFile(ref, representation, locator)));
  server.registerTool("read_files", { description: "Read multiple Files in request order with per-item not_found or invalid_locator results. Omitted representation means auto; textual reads expose line coordinates, PDF reads preserve page coordinates, and locators are intended for bounded follow-up reads and citations. Images currently return metadata only.", inputSchema: { reads: z.array(z.object({ ref: z.string(), representation: z.string().optional(), locator: locator.optional() })).min(1).max(100) }, outputSchema: { results: z.array(z.record(z.string(), z.unknown())) }, annotations: readAnnotations }, ({ reads }) => safe("results", () => api.readFiles(reads)));
  server.registerTool("list_archive", { description: "Browse the structure of one canonical archive File without promoting or exploding members into Files. Archive paths are exact member paths; path optionally scopes to a directory/prefix, depth and limit bound traversal, and the result is format-agnostic.", inputSchema: { ref: z.string(), path: z.string().optional(), depth: z.number().int().nonnegative().max(100).optional(), limit: z.number().int().positive().max(100).optional() }, outputSchema: { entries: z.array(archiveEntry) }, annotations: readAnnotations }, ({ ref, ...input }) => safe("entries", () => api.listArchive(ref, input)));
  server.registerTool("read_archive_entry", { description: "Read one exact member of one canonical archive File without creating a File. Text members use bounded line coordinates, PDFs preserve page coordinates, images return metadata only, and binary members are never coerced to text. Nested archives remain entries until explicitly read or extracted.", inputSchema: { ref: z.string(), path: z.string(), representation: z.string().optional(), locator: locator.optional() }, outputSchema: { read: z.record(z.string(), z.unknown()) }, annotations: readAnnotations }, ({ ref, path, representation, locator }) => safe("read", () => api.readArchiveEntry(ref, path, representation, locator)));
