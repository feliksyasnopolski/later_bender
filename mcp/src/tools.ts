...
const taskSummary = z.object({ id: z.number(), project: projectSummary, title: z.string(), status: statuses, position: z.number(), priority: priorities, tags: z.array(z.string()), related_note_ids: z.array(z.number()), created_at: timestamp, updated_at: timestamp });
...
export function registerTools(server: McpServer, api: LaterBenderApi): void {
  ...
  server.registerTool("create_project", { description: "Create a project. Omit slug to let Later Bender generate one from the name.", inputSchema: { name: z.string(), slug: z.string().optional(), description: z.string().optional() }, outputSchema: projectMutationOutput, annotations: createAnnotations }, (input) => compactMutation("project", () => api.createProject(input)));
